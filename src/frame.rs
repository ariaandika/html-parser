...
type WebSocketError = io::Error;

fn io<E: Into<Box<dyn std::error::Error + Send + Sync>>>(err: E) -> io::Error {
...
impl<'f> Frame<'f> {
    ...
    pub fn writev<S>(&mut self, stream: &mut S) -> Result<(), std::io::Error>
    where
        S: Write + Unpin,
    {
        use std::io::IoSlice;
        ...
        let mut n = stream.write_vectored(&b)?;
        if n == total {
            return Ok(());
        }
        ...
        while n <= size {
            ...
            n += stream.write_vectored(&b)?;
        }

        // Header out of the way.
        if n < total && n > size {
            stream.write_all(&self.payload[n - size..])?;
        }
